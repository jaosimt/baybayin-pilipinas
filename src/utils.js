...
export const ellipses = (string, length) => {
    ...
    return ret.join(' ').replace(/[\W]$/, '') + '...'
};

export const ___bybyn = {
    a: 'ᜀ',
    e: 'ᜁ',
    i: 'ᜁ',
    o: 'ᜂ',
...
export const translate = (str, normalize) => {
    str = str.toString();
    if (str.trim() === '') return;
    
    for (let b in ___ynbyby) {
        if (___ynbyby.hasOwnProperty(b)) str = str.replace(new RegExp(b, 'igm'), ___ynbyby[b]);
    }
    
    return (normalize === true ? str.replace(/\[e\/i\]/g, 'i').replace(/\[o\/u\]/g, 'o') : str).replace(/(\[\w\/\w\])/gm, "<span class='dimmed'>$1</span>").replace(/\n/gm, '<br/>');
};